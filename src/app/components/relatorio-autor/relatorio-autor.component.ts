...
export class RelatorioAutorComponent {
  ...
  async carregarDados() {
    ...
    this.dadosRelatorio.forEach((item) => {
      ...
      if (autor) {
        const livroAutor: LivroModel ={ codl: 0, titulo: item.titulo, editora: item.editora, edicao: item.edicao, anoPublicacao: item.anoPublicacao, assuntos:[], autores: [], precos:[]} ;
